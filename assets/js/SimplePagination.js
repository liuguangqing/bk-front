/**
 * 简单分页类  newnewnewnewnewnewnewnewnewnewnewnewnewnewnewnewnewnewnewnewnewnewnew
 */
export class SimplePagination {
  ...
  init(paramsObj) {
    ...
    ;(state.pCName = paramsObj.pCName || 'page-li'),
      // 元素显示的 Dom
      (state.btnDom = paramsObj.btnDom || 'li'),
      // 路由跳转
      (state.routerhead = paramsObj.routerhead || ''),
      // 当选中页码时添加的类名class
    ...
    state.totalPageCount > state.maxShowBtnCount + 2 &&
  ...
  switchPage() {
    ...
    pCNameList.forEach(item => {
      item.addEventListener(state.swEvent, e => {
        ...
        // let dataNumberAttr = isNaN(currentPageEle.innerHtml)? currentPageEle.getAttribute(state.dataNumberAttr).split(state.routerhead)[1]
        let dataNumberAttr = !isNaN(currentPageEle.innerHTML)
          ? currentPageEle
              .getAttribute(state.dataNumberAttr)
              .split(state.routerhead)[1]
          : currentPageEle.getAttribute(state.dataNumberAttr)
        if (dataNumberAttr) {
          // 点击 数字 按钮
  ...
  gotoPage(pageNumber) {
    setTimeout(() => {
      ...
      // 清除 active 样式
      this.removeClass(
        this.selectorEle(`.${state.pCName}.${state.activeCName}`),
        state.activeCName
      )
      if (state.activePosition) {
        ...
        if (pageNumber > state.maxShowBtnCount && pageNumber < rEllipseSign) {
          ...
          for (let i = 1; i < state.maxShowBtnCount + 1; i++) {
            ...
            evaNumberLi[i].setAttribute(
              state.dataNumberAttr,
              state.routerhead + (pageNumber + (i - state.activePosition)) * 1
            )
          }
          console.log(evaNumberLi[state.activePosition])
          ...
          if (
            ...
            for (let i = 1; i < state.maxShowBtnCount + 1; i++) {
              ...
              evaNumberLi[i].setAttribute(
                state.dataNumberAttr,
                state.routerhead +
                  (state.totalPageCount - (state.maxShowBtnCount - i) - 1) * 1
              )
            }
          }
          this.addClass(
            evaNumberLi[
              state.maxShowBtnCount + 1 - (state.totalPageCount - pageNumber)
            ],
            state.activeCName
          )
        }
  ...
  renderPageDOM() {
    ...
    if (totalPageCount - 2 > maxShowBtnCount) {
      paginationStr += `
      <${btnDom} class="${pCName} number-ellipsis ellipsis-head" style="display: none;">...</${btnDom}>`
      for (let i = 2; i < maxShowBtnCount + 2; i++) {
        paginationStr += `<${btnDom} class="${pCName} ${pageNumberCName} ${
          i === 1 ? activeCName : ''
        }" ${dataNumberAttr}='${routerhead}${i}'>${i}</${btnDom}>`
      }
      paginationStr += `
      <${btnDom} class="${pCName} number-ellipsis ellipsis-tail">...</${btnDom}>
      <${btnDom} class="${pCName} ${pageNumberCName}" ${dataNumberAttr}='${routerhead}${totalPageCount}'>${totalPageCount}</${btnDom}>
      `
    } else {
